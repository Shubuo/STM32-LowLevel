...
//		RCC->APB2ENR |= (1UL << 2);          /* Enable GPIOA clock 				 */
		...
		RCC->APB1ENR = RCC_APB1ENR_TIM3EN;	/* Enable TIM3 | 0x2 */		
		GPIOA->CRL = 0x44 ;  					//Input floating for PA0/PA1
		GPIOC->CRL = 0x3300;						//Output 50Mhz 	 for PC2/PC3
		
		TIM3->PSC = DELAY;			// Prescaler PSC + 1 = 36 (1ms)
//		TIM3->SR  = 0	;
		TIM3->ARR = 1000;				// Auto Reload Value means 1ms*1000 = 1 second
		TIM3->CR1 |= TIM_CR1_CEN;
		
		
		//Interrupt conditions
		//Handler'in içine while yapistirarak interruptsiz da çalistirabilirsin
...
if(TIM3->SR & TIM_SR_UIF) // if UIF flag is set
  ...
  TIM3->SR &= ~TIM_SR_UIF; // clear UIF flag
		if(GPIOA->IDR & 0x2)		//Butona basilirsa
